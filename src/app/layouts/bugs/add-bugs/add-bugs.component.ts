...
export class AddBugsComponent implements OnInit {
		...
		this.route.paramMap.subscribe((paramMap: ParamMap) => {
			if (paramMap.has('id')) {
				...
				this.bugservice.getBugsDetail(this.bugs_id).subscribe((bugData: any) => {
					...
					console.log(bugData,'getbugdata');
					bugData.bugDetails.image.forEach(element => {
					//	console.log(element,'element');
							this.showFile.push(element)
					});
					console.log(this.showFile,'arrr');
    ...
    this.fileArr.forEach((item) => {
      ...
      this.fileObj.push(item.item)
		...
		// let finalArr = [...this.f.fileSource.value,...this.fileObj]
